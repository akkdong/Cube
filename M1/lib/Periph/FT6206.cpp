...
/*!
   ...
   single-touch since the two points cant share the same half of the screen.
    @returns {@link Point} object that has the x and y coordinets set. If the
   z coordinate is 0 it means the point is not touched. If z is 1, it is
   currently touched.
*/
/**************************************************************************/
Point FT6206::getPoint(uint8_t n) {
  readData();
  if ((touches == 0) || (n > 1)) {
    return Point(0, 0, 0);
  } else {
    return Point(touchX[n], touchY[n], 1);
  }
}
...
Point::Point(int16_t _x, int16_t _y, int16_t _z) {
  x = _x;
  y = _y;
  z = _z;
}
...
/**************************************************************************/
/*!
    @brief  Simple == comparator for two Point objects
