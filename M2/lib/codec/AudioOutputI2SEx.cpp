...
AudioOutputI2SEx::AudioOutputI2SEx(int port, int output_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
...
AudioOutputI2SEx::AudioOutputI2SEx(long sampleRate, pin_size_t sck, pin_size_t data) {
    i2sOn = false;
    mono = false;
    bps = 16;
    channels = 2;
...
AudioOutputI2SEx::~AudioOutputI2SEx()
{
  stop();
}

bool AudioOutputI2SEx::SetPinout()
{
  #ifdef ESP32
...
bool AudioOutputI2SEx::SetPinout(int mclk, int bclk, int wclk, int dout)
{
  mclkPin = mclk;
  bclkPin = bclk;
  wclkPin = wclk;
...
bool AudioOutputI2SEx::SetRate(int hz)
{
...
bool AudioOutputI2SEx::SetBitsPerSample(int bits)
{
...
bool AudioOutputI2SEx::SetChannels(int channels)
{
...
bool AudioOutputI2SEx::SetOutputModeMono(bool mono)
{
  this->mono = mono;
  return true;
}

bool AudioOutputI2SEx::SetLsbJustified(bool lsbJustified)
{
  this->lsb_justified = lsbJustified;
  return true;
}

bool AudioOutputI2SEx::begin(bool txDAC)
{
  #ifdef ESP32
    if (!i2sOn)
    {
...
bool AudioOutputI2SEx::ConsumeSample(int16_t sample[2])
{
