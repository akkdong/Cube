...
const char tagRMC[] = {"GPRMC"};
const char tagGGA[] = {"GPGGA"};
...
{
    while (mDataSourcePtr->available())
    {
        //
		...
		else
		{
			...
			else if (mParseStep == NMEA_TAG_SIZE + 2) // checksum high-nibble
			{
				...
				if (n != ((mParity & 0xF0) >> 4)) // bad checksum
				{
					...
					#if DEBUG_PARSING
                    trace_printf("Bad checksum #1: %Xh compared with %c\n", mParity, c);
					#endif
				}
				else
				{
			...
			else if (mParseStep == NMEA_TAG_SIZE + 3) // 7, checksum low-nibble
			{
				...
				if (n != (mParity & 0x0F)) // bad checksum
				{
					...
					#if DEBUG_PARSING
                    trace_printf("Bad checksum #2: %Xh compared with %c\n", mParity, c);
					#endif
				}
				else
				{
			...
			else if (mParseStep == NMEA_TAG_SIZE + 4) // 8, carriage-return
			{
				if (c != '\r')
				{
					mParseStep = -1;
					...
					#if DEBUG_PARSING
					trace_printf("Bad carriage return\n");
					#endif
				}
				else
				{
			...
			else if (mParseStep == NMEA_TAG_SIZE + 5) // 9, newline
			{
				if (c != '\n')
				{
					...
					#if DEBUG_PARSING
					trace_printf("Bad newline\n");
					#endif
				}
				else
				{
					#if DEBUG_PARSING
					trace_puts("[complete a setence: ");
					mDataQueue.dumpReserve();
					trace_printf("]\n");
					trace_printf("== mParseState = %Xh\n", mParseState);
					#endif
					...
					if (IS_SET(mParseState, PARSE_RMC))
					{
						...
						{
							#if DEBUG_PARSING
							trace_puts("Unmatched RMC: throw previous GGA\n");
							#endif
					...
					if (IS_SET(mParseState, PARSE_GGA))
					{
						...
						{
							#if DEBUG_PARSING
							trace_puts("Unmatched GGA: throw previous RMC\n");
							#endif
...
int LocationParser::timeStr2TmStruct(struct tm * _tm, int startPos)
{
	//
	char str[6];
	...
	_tm->tm_min = _tm->tm_min + (int)(mTimeZone * 60);

	return 0;
}

int LocationParser::dateStr2TmStruct(struct tm * _tm, int startPos)
{
	//
	char str[6];
...
void LocationParser::parseField(int fieldIndex, int startPos)
{
...
float LocationParser::strToFloat(int startPos, int limit)
{
...
long LocationParser::strToNum(int startPos, int limit)
{
	long value = 0;
...
long LocationParser::floatToCoordi(float value)
{
