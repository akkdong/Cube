...
function makePrefItem (item) {
    console.log(`makePrefItem: ${JSON.stringify(item)}`)
    ...
    if (item.type === "boolean") {
       ...
       _value.append($('<span>').text(item.desc));
       _value.append($('<span>', { style: 'position: absolute; right:0.4em;'}).append(_label));
...
function savePref () {
    console.log("save preferences...", pref_config);
    $.ajax({
        url: "/Update/config.json",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify(pref_config),        
    }).done(function () {
