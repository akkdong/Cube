...
// HID
#define HID_VID         0x05ac
#define HID_PID         0x820a
#define HID_VERSION     0x0210
...
BLEVario::BLEVario(std::string deviceName, std::string deviceManufacturer, uint8_t batteryLevel) 
...
void BLEVario::begin(void)
{
    ...
    hidDevice->manufacturer()->setValue(deviceManufacturer);
    ...
    // UART
    uartService = bleServer->createService(UART_SERVICE_UUID);

    // Create a BLE Characteristic
    pTxCharacteristic = uartService->createCharacteristic(CHARACTERISTIC_UUID_TX, BLECharacteristic::PROPERTY_NOTIFY);
    pTxCharacteristic->addDescriptor(new BLE2902());

    pRxCharacteristic = uartService->createCharacteristic(CHARACTERISTIC_UUID_RX, BLECharacteristic::PROPERTY_WRITE);
    ...
    BLEAdvertising* hidAdvertising = bleServer->getAdvertising();
    hidAdvertising->setAppearance(HID_KEYBOARD);
    hidAdvertising->addServiceUUID(hidDevice->hidService()->getUUID());
    hidAdvertising->addServiceUUID(UART_SERVICE_UUID);
    hidAdvertising->setScanResponse(false);
    hidAdvertising->start();
...
void BLEVario::setBatteryLevel(uint8_t level) 
{
...
void BLEVario::setName(std::string deviceName) 
{
...
void BLEVario::setDelay(uint32_t ms) 
{
    this->_delay_ms = ms;
}
#endif

void BLEVario::setVendorID(uint16_t vid) 
{ 
    this->vid = vid; 
}

void BLEVario::setProductID(uint16_t pid) 
{ 
    this->pid = pid; 
}

void BLEVario::setVersion(uint16_t version) 
{ 
    this->version = version; 
}

void BLEVario::sendReport(KeyReport* keys)
{
    if (this->isConnected())
    {
...
void BLEVario::sendReport(MediaKeyReport* keys)
{
    if (this->isConnected())
    {
...
size_t BLEVario::press(uint8_t k)
{
	uint8_t i;
...
size_t BLEVario::press(const MediaKeyReport k)
{
...
size_t BLEVario::release(uint8_t k)
{
    uint8_t i;
    if (k >= 136) 			// it's a non-printing key (not a modifier)
    {
...
size_t BLEVario::release(const MediaKeyReport k)
{
    uint16_t k_16 = k[1] | (k[0] << 8);
    uint16_t mediaKeyReport_16 = _mediaKeyReport[1] | (_mediaKeyReport[0] << 8);
    mediaKeyReport_16 &= ~k_16;
...
size_t BLEVario::sendKey(uint8_t c)
{
...
size_t BLEVario::sendKey(const MediaKeyReport c)
{
...
size_t BLEVario::sendKey(const uint8_t *buffer, size_t size) 
{
    size_t n = 0;
...
void BLEVario::onDisconnect(BLEServer* pServer) 
{
...
void BLEVario::onWrite(BLECharacteristic* pCharacteristic) 
{
...
#if defined(USE_NIMBLE)
void BLEVario::delay_ms(uint64_t ms) 
{
...
// UART
int BLEVario::available()
{
    if (bufRear < bufFront)
        return bufFront - bufRear;

    if (bufRear > bufFront)
        return 32 - bufRear + bufFront;
...
size_t BLEVario::write(uint8_t c)
{
    if (this->connected)
    {
